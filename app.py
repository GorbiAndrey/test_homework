...
def load_obj_from_json(filename):
    current_path = str(os.path.dirname(os.path.abspath(__file__)))
    current_file = os.path.join(current_path, filename)
    with open(current_file, 'r', encoding='utf-8') as opened_file:
        return json.load(opened_file)


def init_app(obj_documents=None, obj_directories=None):
    global documents
    if not obj_documents:
        documents = load_obj_from_json('fixtures/documents.json')
    else:
        documents = obj_documents

    global directories
    if not obj_directories:
        directories = load_obj_from_json('fixtures/directories.json')
    else:
        directories = obj_directories
